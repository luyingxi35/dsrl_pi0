#!/bin/bash
# Evaluate pi0.5_droid checkpoint in ManiSkill simulation.
# pi0.5 uses the same flow-matching architecture and pi0_droid openpi config as pi0;
# only the checkpoint path differs.
#
# Usage:
#   bash examples/scripts/sim/eval_pi0_5.sh
#   bash examples/scripts/sim/eval_pi0_5.sh --eval_episodes 3 --max_rollout_steps 100

set -euo pipefail

cd "$(dirname "$0")/../../.."   # → repo root (3 levels up from scripts/sim/)
