"""CLI entry point for train_dino_pi05.py (pi0.5, sparse reward).

Runs ManiSkill PegInsertionVertical via robofac subprocess + pi05_droid + StateSAC + DINOv2.

Key difference from launch_train_dino.py:
...
from examples.sim.train_dino_pi05_delta import main
from jaxrl2.utils.launch_util import parse_training_args


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="DSRL sim training pi0.5 delta-action (pd_joint_delta_pos) (sparse): ManiSkill subprocess + pi05_droid + StateSAC + DINOv2"
    )
    ...
    parser.add_argument("--wandb_project",        default="DSRL_pi05_SimDino")
    ...
    parser.add_argument("--action_scale",         default=0.5,    type=float,
                        help="Scale on DROID max_joint_delta=0.2 rad/step.")
