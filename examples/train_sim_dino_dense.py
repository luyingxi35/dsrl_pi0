#!/usr/bin/env python
"""Simulation DSRL training (dense reward): PegInsertionVertical + pi0_droid + StateSAC + DINOv2.

Dense reward = binary -1/0 reward + Robometer progress(t).
See train_utils_sim_dino_dense.py for reward implementation.
...
from examples.train_utils_sim_dino_dense import (
    STATE_DIM,
    PI0_NOISE_DIM,
...
def main(variant):
    ...
    # Launched after JAX/SAC to let JAX claim its CUDA context first.
    # The server loads Qwen3-VL-4B (~30 s); training is blocked until ready.
    ...
    # ── Ensure subprocess cleanup on any exit ────────────────────────────────────
